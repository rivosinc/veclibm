...
//Two regions of approximation: left is [0, 0x1.7p-1) and right is [0x1.7p-1, 1)
//Both are done with rational functions.
//For left, x*P(x)/Q(x) x in [0, 0x1.7p-1)
//For right, y*P(t)/Q(t), y = sqrt(-log(1-x)); and t = 1/y
...
void F_VER1(API) {
  ...
  for (; _inarg_n > 0; _inarg_n -= vlen) {
    ...
    P = PSTEP_I_ab(x_in_right, P_right_6, P_left_6, T, 
        ...
        PSTEP_I_ab(x_in_right, P_right_9, P_left_9, T, P,
        vlen), vlen), vlen), vlen); 
    ...
    Q = PSTEP_I_ab(x_in_right, Q_right_6, Q_left_6, T, 
        ...
        PSTEP_I_ab(x_in_right, Q_right_9, Q_left_9, T, Q,
        vlen), vlen), vlen), vlen); 
        
    P = PSTEP_I_ab(x_in_right, P_right_0, P_left_0, T, 
        PSTEP_I_ab(x_in_right, P_right_1, P_left_1, T, 
        ...
        PSTEP_I_ab(x_in_right, P_right_5, P_left_5, T, P,
        vlen), vlen), vlen), vlen), vlen), vlen);

    Q = PSTEP_I_ab(x_in_right, Q_right_0, Q_left_0, T, 
        PSTEP_I_ab(x_in_right, Q_right_1, Q_left_1, T, 
        ...
        PSTEP_I_ab(x_in_right, Q_right_5, Q_left_5, T, Q,
        vlen), vlen), vlen), vlen), vlen), vlen);
