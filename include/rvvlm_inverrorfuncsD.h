...
// P_coefficients in asending order, in varying scales. p0_delta is in floating point 
...
// Q_coefficients in asending order, in varying scales. q0_delta is in floating point 
...
#define NEG_LOGX_4_TRANSFORM(vx, n_adjust, y_hi, y_lo, vlen) \
  do { \
    ...
    VFLOAT vx_in = (vx); \
    VINT n = __riscv_vadd(__riscv_vsra(F_AS_I(vx_in), MAN_LEN - 8, (vlen)), 0x96, \
                     vlen); \
    n = __riscv_vsub(__riscv_vsra(n, 8, vlen), EXP_BIAS, vlen); \
    VFLOAT scale = I_AS_F(__riscv_vsll(__riscv_vrsub(n, EXP_BIAS, (vlen)), MAN_LEN, (vlen))); \
    ...
    VFLOAT delta_denom = __riscv_vfadd(__riscv_vfrsub(denom, fp_posOne, (vlen)), vx_in, (vlen)); \
    ...
    VFLOAT poly_right = PSTEP(0x1.74681ff881228p-14, rsq, \
                            PSTEP( 0x1.39751be23e4a3p-16, \
                                   0x1.30a893993e73dp-18, rsq, vlen), \
                        vlen); \
    VFLOAT poly_left = PSTEP(0x1.999999996ce82p-7, rsq, \
                            PSTEP(0x1.249249501b1adp-9, \
                                  0x1.c71c47e7189f6p-12, rsq, vlen), \
                       vlen); \
...
#define ERFCINV_PQ_TINY(T, p_hi_tiny, p_lo_tiny, q_hi_tiny, q_lo_tiny, vlen) \
  do { \
    /* T is in scale of 65 */ \
    VINT P, Q; \
    P = PSTEP_I_SRA(P_tiny_7, T, 4, PSTEP_I_SRA(P_tiny_8, P_tiny_9, 4, T, \
        (vlen)), (vlen)); \
    /* P in Q_56 */ \
    P = PSTEP_I_SRA(P_tiny_5, T, 1, PSTEP_I_SRA(P_tiny_6, P, 2, T, \
        (vlen)), (vlen)); \
    /* P in Q_57 */ \
    P = PSTEP_I_SRA(P_tiny_3, T, 1, PSTEP_I(P_tiny_4, P, T, \
        (vlen)), (vlen)); \
    /* P in Q_60 */ \
    P = PSTEP_I_SLL(P_tiny_1, T, 1, PSTEP_I_SRA(P_tiny_2, P, 1, T, \
        (vlen)), (vlen)); \
    /* P in Q_64 */ \
    P = PSTEP_I(P_tiny_0, T, P, (vlen));\
    /* P in Q_66 */ \
    \
    Q = PSTEP_I_SRA(Q_tiny_7, T, 2, PSTEP_I_SRA(Q_tiny_8, Q_tiny_9, 3, T, \
        (vlen)), (vlen)); \
    /* Q in Q_56 */ \
    Q = PSTEP_I_SRA(Q_tiny_5, T, 1, PSTEP_I_SRA(Q_tiny_6, Q, 2, T, \
        (vlen)), (vlen)); \
    /* Q in Q_57 */ \
    Q = PSTEP_I_SRA(Q_tiny_3, T, 2, PSTEP_I(Q_tiny_4, Q, T, \
        (vlen)), (vlen)); \
    /* P in Q_59 */ \
    Q = PSTEP_I_SLL(Q_tiny_1, T, 1, PSTEP_I(Q_tiny_2, Q, T, \
        (vlen)), (vlen)); \
    /* Q in Q_64 */ \
